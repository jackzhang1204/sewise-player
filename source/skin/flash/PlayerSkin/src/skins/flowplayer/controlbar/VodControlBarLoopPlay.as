...
package skins.flowplayer.controlbar {
	...
	public class VodControlBarLoopPlay extends MovieClip {
		...
		public function VodControlBarLoopPlay() {
			//_flowplayer = this.parent as VodOrange;
			_flowplayer = this.parent as MovieClip;
...
/**------------------------------ 本对象大小及显示设置方法 ---------------------------*/
		...
		public function resize(w:Number,h:Number):void{
			...
			{
				subtitlesBtn.x = normalBtn.x - subtitlesBtn.btn.width;
			...
			{
				clarityBtn.x = rightRefBtn.x - clarityBtn.width;
				soundCloseBtn.x = clarityBtn.x - soundCloseBtn.width;
				if(playTime.visible){
					progressLineW = w - refBtnNum * fullBtn.width - playTime.width - 2 * 6;
				}else{
					progressLineW = w - refBtnNum * fullBtn.width - 2 * 6;
				}
			}else{
				soundCloseBtn.x = rightRefBtn.x - soundCloseBtn.width;
				if(playTime.visible){
					progressLineW = w - refBtnNum * fullBtn.width - playTime.width - 2 * 6;
				}else{
					progressLineW = w - refBtnNum * fullBtn.width - 2 * 6;
		...
		public function show():void{
			//jack zhang fix////
			//var _showTween:Tween = new Tween(this, "y", Regular.easeOut, this.y, _flowplayer.bg.height - this.height, 0.5, true);
			var _showTween:Tween = new Tween(this, "y", Regular.easeOut, this.y, _flowplayer.bg.height - this.controlBarBg.height, 0.3, true);
			_showTween.start();
		}
		
		public function hide():void{
			var _hideTween:Tween  = new Tween(this, "y", Regular.easeOut, this.y, _flowplayer.bg.height, 0.6, true);
			_hideTween.start();
		}
...
/**------------------------------ 私有属性及方法 ---------------------------*/
		...
		//顶级对象
		//private var _flowplayer:VodOrange;
		private var _flowplayer:MovieClip;
		//当前应用宽度
		...
		private function tipShowHandler(e:MouseEvent):void{
			var btnY:Number = this.y + playBtn.y -_flowplayer.tipBubble.height - 3;
			var volumeLineShowTween:Tween;
			
			switch(e.target){
				case clarityBtn:
					_flowplayer.tipBubble.showInfo(clarity_string, clarityBtn.x + clarityBtn.width/2, btnY, _flowplayer.x, _curW);
					break;
				case soundOpenBtn:
					_flowplayer.tipBubble.showInfo(soundOn_string, soundOpenBtn.x + soundOpenBtn.width/2, btnY, _flowplayer.x, _curW);
					...
					break;
				case soundCloseBtn:
					_flowplayer.tipBubble.showInfo(soundOff_string, soundCloseBtn.x + soundCloseBtn.width/2, btnY, _flowplayer.x, _curW);
					...
					break;
				case stopBtn:
					_flowplayer.tipBubble.showInfo(stop_string, stopBtn.x + stopBtn.width/2, btnY, _flowplayer.x, _curW);
					break;
				case pauseBtn:
					_flowplayer.tipBubble.showInfo(pause_string, pauseBtn.x + pauseBtn.width/2, btnY, _flowplayer.x, _curW);
					break;
				case playBtn:
					_flowplayer.tipBubble.showInfo(play_string, playBtn.x + playBtn.width/2, btnY, _flowplayer.x, _curW);
					break;
				case normalBtn:
					_flowplayer.tipBubble.showInfo(normalScreen_string, normalBtn.x + normalBtn.width/2, btnY, _flowplayer.x, _curW);
					break;
				case fullBtn:
					_flowplayer.tipBubble.showInfo(fullScreen_string, fullBtn.x + fullBtn.width/2, btnY, _flowplayer.x, _curW);
					break;
				...
				case subtitlesBtnBtn:
					_flowplayer.tipBubble.showInfo(subtitles_string, subtitlesBtn.x + subtitlesBtnBtn.width / 2, btnY, _flowplayer.x, _curW);
					break;
				//2013.12.6 jackzhang/////////////
		...
		private function timeTipHanlder(e:MouseEvent):void{
			var progressLineY:Number = this.y + progressLine.y -_flowplayer.tipBubble.height - 2;
			...
			//_flowplayer.tipBubble.showInfo(xTime, progressLine.mouseX, progressLineY, _flowplayer.x, _curW);
			_flowplayer.tipBubble.showInfo(xTime, progressLine.mouseX + progressLine.x, progressLineY, _flowplayer.x, _curW);
		...
		private function volumeTipHanlder(e:MouseEvent):void{
			var volumeLineY:Number = this.y + volumeLine.y -_flowplayer.tipBubble.height - 2;
			
			//_flowplayer.tipBubble.showInfo(String(volumeLine.mouseX), progressLine.mouseX, volumeLineY, _flowplayer.x, _curW);
			
			var value:Number = Math.round((volumeLine.mouseX - volumeLine.volumeDragger.x) / volumeLine.volumeDragger.width * 100);
			if(value >=0 && value <= 100)
			{
				//_flowplayer.tipBubble.showInfo(String(value), progressLine.mouseX, volumeLineY, _flowplayer.x, _curW);
				_flowplayer.tipBubble.showInfo(String(value), volumeLine.x + volumeLine.width / 2, volumeLineY - volumeLine.height, _flowplayer.x, _curW);
			}
		}
		...
		private function tipHideHanlder(e:MouseEvent):void{
			_flowplayer.tipBubble.visible = false;
