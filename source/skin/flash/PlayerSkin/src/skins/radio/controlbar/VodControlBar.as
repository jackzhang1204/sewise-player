 ...
 ********************************************************************************/
package skins.radio.controlbar {
	...
	public class VodControlBar extends MovieClip {
		...
		public function VodControlBar() {
			//_white = this.parent as VodOrange;
...
/**------------------------------ 本对象大小及显示设置方法 ---------------------------*/
		...
		public function resize(w:Number,h:Number):void{
			...
			if(clarityBtn.visible)
			{
				...
				if(playTime.visible){
					progressLineW = w - (refBtnNum - 1) * fullBtn.width - playTime.width - 2 * 6;
				}else{
					progressLineW = w - (refBtnNum - 1) * fullBtn.width - 2 * 6;
				}
			}
		...
		private function tipShowHandler(e:MouseEvent):void{
			...
			switch(e.target){
				...
				case soundCloseBtn:
					...
					break;
				case stopBtn:
					_white.tipBubble.showInfo(stop_string, stopBtn.x + stopBtn.width/2, btnY, _white.x, _curW);
					break;
				case pauseBtn:
				...
				case subtitlesBtnBtn:
					_white.tipBubble.showInfo(subtitles_string, subtitlesBtn.x + subtitlesBtnBtn.width / 2, btnY, _white.x, _curW);
					break;
				//2013.12.6 jackzhang/////////////
