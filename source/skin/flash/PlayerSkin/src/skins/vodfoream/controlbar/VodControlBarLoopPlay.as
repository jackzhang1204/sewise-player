...
/**------------------------------ 私有属性及方法 ---------------------------*/
		...
		private function tipShowHandler(e:MouseEvent):void{
			...
			switch(e.target){
				case clarityBtn:
					_vodfoream.tipBubble.showInfo(clarity_string, clarityBtn.x + clarityBtn.width/2, btnY, _vodfoream.x, _curW);
					break;
				case soundOpenBtn:
		...
		private function volumeTipHanlder(e:MouseEvent):void{
			...
			//_vodfoream.tipBubble.showInfo(String(volumeLine.mouseX), progressLine.mouseX, volumeLineY, _vodfoream.x, _curW);
			
			var value:Number = Math.round((volumeLine.mouseX - volumeLine.volumeDragger.x) / volumeLine.volumeDragger.width * 100);
			if(value >=0 && value <= 100)
			{
				//_vodfoream.tipBubble.showInfo(String(value), progressLine.mouseX, volumeLineY, _vodfoream.x, _curW);
				_vodfoream.tipBubble.showInfo(String(value), volumeLine.x + volumeLine.width / 2, volumeLineY - volumeLine.height, _vodfoream.x, _curW);
			}
		}
