...
package skins.vspaas.controlbar {
	...
	public class VodControlBarLoopPlay extends MovieClip {
		...
		public function VodControlBarLoopPlay() {
			//_vspaas = this.parent as VodOrange;
...
/**------------------------------ 本对象大小及显示设置方法 ---------------------------*/
		...
		public function show():void{
			//jack zhang fix////
			//var _showTween:Tween = new Tween(this, "y", Regular.easeOut, this.y, _vspaas.bg.height - this.height, 0.5, true);
			var _showTween:Tween = new Tween(this, "y", Regular.easeOut, this.y, _vspaas.bg.height - this.controlBarBg.height, 0.3, true);
			_showTween.start();
		}
		
		public function hide():void{
			var _hideTween:Tween  = new Tween(this, "y", Regular.easeOut, this.y, _vspaas.bg.height, 0.6, true);
			_hideTween.start();
		}
...
/**------------------------------ 私有属性及方法 ---------------------------*/
		...
		public function toFullScreen():void{
			...
			var btnY:Number = this.y + playBtn.y -_vspaas.tipBubble.height - 3;
			_vspaas.tipBubble.showInfo(fullScreen_string, fullBtn.x + fullBtn.width/2, btnY, _vspaas.x, _curW);
			var myTimer:Timer = new Timer(2000, 1);
			myTimer.addEventListener(TimerEvent.TIMER, timerHandler);
			myTimer.start();
			function timerHandler(event:TimerEvent):void{
				_vspaas.tipBubble.visible = false;
		...
		private function tipShowHandler(e:MouseEvent):void{
			...
			switch(e.target){
				case clarityBtn:
					_vspaas.tipBubble.showInfo(clarity_string, clarityBtn.x + clarityBtn.width/2, btnY, _vspaas.x, _curW);
					break;
				case soundOpenBtn:
					_vspaas.tipBubble.showInfo(soundOn_string, soundOpenBtn.x + soundOpenBtn.width/2, btnY, _vspaas.x, _curW);
					...
					break;
				case soundCloseBtn:
					_vspaas.tipBubble.showInfo(soundOff_string, soundCloseBtn.x + soundCloseBtn.width/2, btnY, _vspaas.x, _curW);
					...
					break;
				case stopBtn:
					_vspaas.tipBubble.showInfo(stop_string, stopBtn.x + stopBtn.width/2, btnY, _vspaas.x, _curW);
					break;
				case pauseBtn:
					_vspaas.tipBubble.showInfo(pause_string, pauseBtn.x + pauseBtn.width/2, btnY, _vspaas.x, _curW);
					break;
				case playBtn:
					_vspaas.tipBubble.showInfo(play_string, playBtn.x + playBtn.width/2, btnY, _vspaas.x, _curW);
					break;
				case normalBtn:
					_vspaas.tipBubble.showInfo(normalScreen_string, normalBtn.x + normalBtn.width/2, btnY, _vspaas.x, _curW);
					break;
				case fullBtn:
					_vspaas.tipBubble.showInfo(fullScreen_string, fullBtn.x + fullBtn.width/2, btnY, _vspaas.x, _curW);
					break;
				...
				case subtitlesBtnBtn:
					_vspaas.tipBubble.showInfo(subtitles_string, subtitlesBtn.x + subtitlesBtnBtn.width / 2, btnY, _vspaas.x, _curW);
					break;
				//2013.12.6 jackzhang/////////////
		...
		private function timeTipHanlder(e:MouseEvent):void{
			var progressLineY:Number = this.y + progressLine.y -_vspaas.tipBubble.height - 2;
			...
			//_vspaas.tipBubble.showInfo(xTime, progressLine.mouseX, progressLineY, _vspaas.x, _curW);
			_vspaas.tipBubble.showInfo(xTime, progressLine.mouseX + progressLine.x, progressLineY, _vspaas.x, _curW);
		...
		private function volumeTipHanlder(e:MouseEvent):void{
			var volumeLineY:Number = this.y + volumeLine.y -_vspaas.tipBubble.height - 2;
			
			//_vspaas.tipBubble.showInfo(String(volumeLine.mouseX), progressLine.mouseX, volumeLineY, _vspaas.x, _curW);
			
			var value:Number = Math.round((volumeLine.mouseX - volumeLine.volumeDragger.x) / volumeLine.volumeDragger.width * 100);
			if(value >=0 && value <= 100)
			{
				//_vspaas.tipBubble.showInfo(String(value), progressLine.mouseX, volumeLineY, _vspaas.x, _curW);
				_vspaas.tipBubble.showInfo(String(value), volumeLine.x + volumeLine.width / 2, volumeLineY - volumeLine.height, _vspaas.x, _curW);
			}
		}
